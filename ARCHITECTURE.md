...
**Two always-on `setInterval` loops** (the only always-on timers in the repo; a third `setInterval` mention at `src/index.ts` L6257 is a comment describing the oracle poller, not a separate loop):
