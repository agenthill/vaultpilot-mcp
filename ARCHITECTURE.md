...
Both gaps below were CONFIRMED against live code and are now LANDED (fixed together in PR #706) — kept here as the regression record INV-T1's exit criterion continues to guard against:
- **Solana `Connection` — untimed — LANDED (#693 → PR #706; `robustness:solana-conn-no-timeout`).** `modules/solana/rpc.ts` `getSolanaConnection()` builds `new Connection(url, {commitment:"confirmed", fetch: fetchWithRateLimitDetect})`. The shim `fetchWithRateLimitDetect` previously did `await fetch(input, init)` with no `AbortSignal`/timeout, so every Solana RPC call, including `broadcastSolanaTx`, could hang indefinitely. Now fixed: `fetchWithRateLimitDetect` wraps the call in an `AbortController` + 10 s default timeout (`DEFAULT_FETCH_TIMEOUT_MS`), matching `data/http.ts`'s `fetchWithTimeout` pattern. (`robustness:solana-conn-cache-race` — **RESOLVED, non-issue; do not relitigate.** `getSolanaConnection()` rebuilds `cachedConnection` when the resolved URL changes, but the cache-check → rebuild path is fully synchronous — no `await`/yield point sits between the `cachedConnectionUrl === url` check (L146-148) and the `new Connection(...)` rebuild (L152-157) — so on JS's single event loop the URL swap cannot interleave with an in-flight call; that call holds its already-dereferenced `Connection` reference, which stays valid after the module-level variable is reassigned. No race. Verified against `origin/main`; adjudicated and #713 CLOSED. A guard test (PR #728) keeps this RED-safe if a future async gap is ever introduced between the check and the rebuild — see §8.)
- **`4byte.directory` — untimed on the pre-sign path — LANDED (#694 → PR #706; `robustness:fourbyte-no-timeout-presign`).** `data/apis/fourbyte.ts` (19 lines) is reachable unguarded from `signing/verify-decode.ts` (pre-sign calldata cross-check) and `modules/history/decode.ts`. `defaultFetch` previously did raw `(url) => fetch(url)` — no timeout — so a stalled or MITM'd 4byte.directory connection could stall the pre-sign check indefinitely. Now fixed: `defaultFetch` routes through `data/http.ts`'s `fetchWithTimeout` (`DEFAULT_FETCH_TIMEOUT_MS` = 10 s), and an independent `withTimeout` race wrapper bounds every `fetchFn` — default or caller-injected — uniformly, matching the Solana fix's pattern.
...
|---|---|---|---|---|
| Security | `transferFrom` recipient classifier fix + `to==wallet` test | §3.2 | `security/custom-call-classifier.ts`, `modules/custom-call/actions.ts` | #711 — closed |
| Security | tx-store deep `Object.freeze` + mutation regression test | §3.3 | `signing/tx-store.ts` | #710 — closed |
...
**Landed.** Five §4/§5 robustness bugs already shipped (closed) — see the rows above for the doc-section mapping: timeouts #693/#694 → PR #706; api-key leak #695 → #703 (follow-up #707 open); permission conflation #696 → #705; oracle poller #697 → #702; LTC drift #698 → #704. Two §3 Security units also landed: tx-store deep-freeze #710 → PR #730; `transferFrom` recipient classifier #711 → PR #727.
